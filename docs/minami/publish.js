...
function buildNavLink(linkClass, linkContent) {
	return ['<li class="nav-link nav-' + linkClass + '-link">', linkContent, '</li>'].join('');
}
...
exports.publish = function (taffyData, opts, tutorials) {
	...
	data().each(function (doclet) {
		if (doclet.scope != 'global') {
			let names = doclet.longname.split('#');
			doclet.longname = names.slice(1).join('#');
			if (doclet.memberof) {
				let mem = doclet.memberof.split('#');
				mem = mem.slice(1).join('#');
				if (mem) doclet.memberof = mem;
				else doclet.memberof = 'global';
			}
		}
		var url = helper.createLink(doclet);
