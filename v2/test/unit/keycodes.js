describe('_keyCodeFromAlias', function () {
	...
	describe('deprecated aliases', function () {
		...
		it("aliases 'MINUT' to 'MINUS'", function () {
			expect(_keyCodeFromAlias('MINUT')).to.equal(_keyCodeFromAlias('MINUS'));
		});

		it('warns when using MINUT', function () {
			_keyCodeFromAlias('MINUT');
			expect(_warn.firstCall.args[0]).to.equal(
				'Key literal "MINUT" is deprecated and may be removed in a ' +
					'future version of p5play. Please use "MINUS" instead.'
			);
		});

		it("aliases 'COMA' to 'COMMA'", function () {
			expect(_keyCodeFromAlias('COMA')).to.equal(_keyCodeFromAlias('COMMA'));
		});

		it('warns when using COMA', function () {
			_keyCodeFromAlias('COMA');
			expect(_warn.firstCall.args[0]).to.equal(
				'Key literal "COMA" is deprecated and may be removed in a ' +
					'future version of p5play. Please use "COMMA" instead.'
			);
		});
	});
});
