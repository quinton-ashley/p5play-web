...
`sprite.scale` は、スプライトのコライダーと画像の、両方のサイズを変更します。スケール値に 2 を与えると、スプライトのサイズは2倍になります。
...
p5play は、 Box2D 物理エンジンのJS移植版である planck.js を使用しています。このエンジンはリアルなインタラクションを高いパフォーマンスで実現しますが、注意すべき制限もあります。
...
もしプラットフォーマーゲームを作る場合、スプライトがプラットフォームに立っているかどうかの判定に `colliding` 関数を利用するのは、おすすめできません。より良い方法として、私たちの[プラットフォーマーデモ](https://openprocessing.org/sketch/1869796)のコードをチェックしてみてください。
...
このコードサンプルでは、ドローンは重力に逆らって飛ばなければなりません。ドローンを飛ばし、それから落下させ、再度ドローンに上向きの力を加えると、落下は徐々に止まり、飛び始めます！
...
動きのあるスプライトに視覚的エフェクトをかける際、事前にエフェクト用のアニメーションを定義することが困難な場合があります。
...
このコードサンプルで使用している `Turtle` スプライトは、何の変哲もないスプライトです。
緑色で、三角形をしています。
昔懐かしい Turtle のプログラミングを彷彿とさせますね。
