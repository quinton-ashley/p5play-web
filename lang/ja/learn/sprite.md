...
画像の読み込みが完了してからプログラムを開始したい、という場合は、 p5.js の `preload` 関数で [`loadImage`](https://q5js.org/learn/#loadImage) を使用するのがベストです。
...
p5play のカラーパレットは[JavaScript オブジェクト](https://p5js.org/reference/p5/object)形式でなければなりません。シンプルな JS オブジェクトとは、見出しと値のペアの集まりです。ピクセルアートで使用するそれぞれの文字に対し、任意の色を定義することができます。色を作成するには、p5.js の[`color`](https://q5js.org/learn/#color)関数に、 RGB（赤、緑、青）値、または HEX カラーコードを引数として渡します。
...
このコードサンプルでは、 [p5.js dist](https://q5js.org/learn/#dist) 関数が使用されており、プレイヤーとその味方との間の距離を計算しています。
