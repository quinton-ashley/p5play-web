...
スプライトの `physicsType` または `physics` プロパティは、物理シミュレーション内でのコライダーの挙動を決定します。

- `DYNAMIC`/`DYN`（デフォルト）：重力の影響を受ける
- `STATIC`/`STA`：動かない
- `KINEMATIC`/`KIN`：他のスプライトによって動かされない

デフォルトで、スプライトには他のスプライトとの衝突を検出できるコライダーが付いています。

コード例の右上にあるリロードアイコンをクリックすると、もう一度
